...
async function main() {
    ...
    const org = await db_1.default.organization.upsert({
    ...
    const createdUsers = await Promise.all(users.map((user) => db_1.default.user.upsert({
    ...
    const aiManager = createdUsers.find((u) => u.role === 'ai_manager');
    // 3. Create Models
    const modelData = [
        {
    ...
    const createdModels = await Promise.all(modelData.map((model) => db_1.default.model.upsert({
    ...
    const createdPolicies = await Promise.all(policies.map((policy) => db_1.default.policy.upsert({
    ...
    // 5. Create Violations
    const embeddingModel = createdModels.find((m) => m.name === 'embedding-v3');
    const externalApiPolicy = createdPolicies.find((p) => p.name === 'Restrict External API Calls');
    ...
    await Promise.all(budgets.map((b) => db_1.default.budget.create({
        data: {
            ...b,
            organizationId: org.id,
        },
    ...
    await db_1.default.spendRecord.createMany({
        data: spendRecords,
    });
    // 8. Create Workflows
    const workflows = [
    ...
    await Promise.all(workflows.map((wf) => db_1.default.workflow.create({
        data: {
            ...wf,
            organizationId: org.id,
        },
