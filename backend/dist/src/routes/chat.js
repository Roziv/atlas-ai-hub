...
// --- Budget Enforcement ---
async function checkBudget(orgId, department) {
    if (!department)
        return { allowed: true };
    ...
    if (!budget)
        return { allowed: true }; // No budget set, allow for now
...
// --- Governance Scanner ---
async function scanContent(text, data) {
    ...
    for (const policy of policies) {
        ...
        for (const rule of rules) {
            ...
            if (rule.type === 'pii') {
                ...
                if (PII_PATTERNS[rule.subtype]?.test(text))
                    triggered = true;
            }
            else if (rule.type === 'keyword') {
                const matches = rule.match || [];
                if (matches.some((word) => text.toLowerCase().includes(word.toLowerCase()))) {
                    triggered = true;
                }
            }
            if (triggered) {
                ...
                // Trigger Workflows
                await (0, workflows_1.triggerWorkflows)('violation_detected', {
                    orgId: data.orgId,
                    modelId: data.modelId,
                    userId: data.userId,
...
async function executeTool(toolId, params) {
    const tool = await db_1.default.tool.findUnique({ where: { id: toolId } });
    if (!tool)
        throw new Error("Tool not found in library");
    ...
    if (tool.type === 'api') {
        ...
        if (!res.ok)
            return { error: `API Error: ${await res.text()}` };
...
async function recordUsage(data) {
    ...
    const amount = ((data.inputTokens + data.outputTokens) / 1000000) * rate;
    await db_1.default.spendRecord.create({
    ...
    try {
        ...
        if (userId) {
            try {
                ...
                department = user.publicMetadata.department || 'Default';
            }
            catch (e) { }
        }
        let org = await db_1.default.organization.findUnique({ where: { id: req.orgId } });
        if (!org) {
            org = await db_1.default.organization.findUnique({ where: { slug: 'acme-corp' } });
        }
        if (!org)
        ...
        const rule = rules.find((r) => r.department === department) || rules[0];
        const modelDef = library.find((m) => m.id === rule?.modelId) || library[0] || { provider: 'ollama', modelId: 'llama3' };
        ...
        const dbTools = await db_1.default.tool.findMany({ where: { id: { in: selectedToolIds } } });
        ...
        if (provider === 'openai' || provider === 'groq') {
            ...
            if (!response.ok)
                throw new Error(`${provider} Error (${response.status}): ${json.error?.message || JSON.stringify(json)}`);
            ...
            const response = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                body: JSON.stringify({ model: modelId, max_tokens: 1024, system: systemPrompt, messages: messages.map((m) => ({ role: m.role, content: m.content })) })
            });
            const json = await response.json();
