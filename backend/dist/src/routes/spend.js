...
router.get('/', async (req, res) => {
    ...
    if (startDate || endDate) {
        where.recordDate = {};
        if (startDate)
            where.recordDate.gte = new Date(String(startDate));
        if (endDate)
            where.recordDate.lte = new Date(String(endDate));
    }
    try {
        const [spend, total] = await Promise.all([
            db_1.default.spendRecord.findMany({
                where,
                orderBy: { recordDate: 'desc' },
                skip,
                take,
            }),
            db_1.default.spendRecord.count({ where })
        ]);
...
router.get('/stats', async (req, res) => {
    ...
    try {
        ...
        // 1. Total MTD
        const mtdAgg = await db_1.default.spendRecord.aggregate({
        ...
        // 3. Provider Shares
        const providerAgg = await db_1.default.spendRecord.groupBy({
            by: ['provider'],
            where: { organizationId: orgId, recordDate: { gte: startOfMonth } },
            _sum: { amount: true }
        });
        ...
        const dailyAgg = await db_1.default.spendRecord.groupBy({
            by: ['recordDate'],
