...
export default function AnalyticsPage() {
  ...
  const persona = role === 'ai_manager' ? 'manager' : 'finance';
  ...
  return (
    <DashboardShell persona={persona}>
      <PageHeader t={t} title="Enterprise Analytics" subtitle="Adoption, cost, and health trends by department"
        actions={
      ...
      <PageBody t={t}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 20 }}>
          <Card t={t} title="Anomalies by Department" subtitle="Flagged usage patterns">
             <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
                {[
                  { dept: 'Marketing', msg: 'Sudden spike in Claude usage', impact: '+$4.2K', severity: 'warn' },
                  { dept: 'Engineering', msg: 'Unused reserved instances', impact: '-$1.1K', severity: 'info' },
                  { dept: 'Sales', msg: 'Shadow AI detection (Perplexity)', impact: 'Security Risk', severity: 'bad' },
                ].map((a, i) => (
                  <div key={i} style={{ padding: 12, borderRadius: 8, background: t.surfaceAlt, display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ 
                      width: 32, height: 32, borderRadius: 8, 
                      background: a.severity === 'bad' ? t.dangerSoft : a.severity === 'warn' ? t.warnSoft : t.accentSoft,
                      color: a.severity === 'bad' ? t.danger : a.severity === 'warn' ? t.warn : t.accent,
                      display: 'flex', alignItems: 'center', justifyContent: 'center'
                    }}>
                      <Icon name={a.severity === 'bad' ? 'alert' : 'info'} size={16} />
                    </div>
                    <div style={{ flex: 1 }}>
                       <div style={{ fontSize: 12, fontWeight: 700, color: t.text }}>{a.dept}</div>
                       <div style={{ fontSize: 11, color: t.textFaint }}>{a.msg}</div>
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: a.severity === 'bad' ? t.dangerText : t.text }}>{a.impact}</div>
                  </div>
                ))}
             </div>
          </Card>

          <Card t={t} title="Efficiency Trends" subtitle="Cost per 1k tokens over time">
             <Sparkline t={t} data={[30, 45, 35, 50, 40, 60, 55, 70, 65, 80]} height={180} />
          </Card>
        </div>
      </PageBody>
    </DashboardShell>
